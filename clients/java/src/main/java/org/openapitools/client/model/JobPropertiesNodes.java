...
public class JobPropertiesNodes extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(JobPropertiesNodes.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!JobPropertiesNodes.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'JobPropertiesNodes' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<JobPropertiesNodes>() {
                @Override
                public void write(JsonWriter out, JobPropertiesNodes value) throws IOException {
                ...
                @Override
                public JobPropertiesNodes read(JsonReader in) throws IOException {
                    ...
                    if (match == 1) {
                        JobPropertiesNodes ret = new JobPropertiesNodes();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonElement));
                        return ret;
                    }

                    throw new IOException(String.format("Failed deserialization for JobPropertiesNodes: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public JobPropertiesNodes() {
        super("oneOf", Boolean.FALSE);
    }

    public JobPropertiesNodes(Integer o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }

    public JobPropertiesNodes(List<Integer> o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }
    ...
    public Map<String, Class<?>> getSchemas() {
        return JobPropertiesNodes.schemas;
    }
  ...
  * @throws IOException if the JSON Element is invalid with respect to JobPropertiesNodes
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
    ...
    if (validCount != 1) {
      throw new IOException(String.format("The JSON string is invalid for JobPropertiesNodes with oneOf schemas: Integer, List<Integer>. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
    }
  }

 /**
  * Create an instance of JobPropertiesNodes given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of JobPropertiesNodes
  * @throws IOException if the JSON string is invalid with respect to JobPropertiesNodes
  */
  public static JobPropertiesNodes fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, JobPropertiesNodes.class);
  }

 /**
  * Convert an instance of JobPropertiesNodes to an JSON string
  *
  * @return JSON string
  */
